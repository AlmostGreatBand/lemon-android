...
import com.agb.data.remote.RegistrationRemoteDataSource
...
val dataModule = module {
    ...
    single { getAuthService<RegistrationApi>(get()) }
    single<RegistrationDataSource> { RegistrationRemoteDataSource(get()) }

    single<UserRepository> { UserRepositoryImpl(get(local), get(remote), get(), get()) }
